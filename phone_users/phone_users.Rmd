...
# Train a MARS model with bagEarthGCV using Caret
# Grid search for the best degree
mars_model <- caret::train(y ~ ., data = marsDf, method = "gcvEarth", trControl = trainControl(method = "cv", number = 10), tuneGrid = data.frame(degree = 1:5))
summary(mars_model)
# Visualize the MARS model
plot(mars_model)
...
combinedModel <- function(row) {
  y = predict(prunedTree, newdata = row, type = "class")
