...
set.seed(1234)
train_index <- createDataPartition(train$y_binary, p = 0.7, list = FALSE)
tree.train <- train[train_index, ]
tree.test <- train[-train_index, ]
...
rTreeFit <- tree(y_binary ~ ., data = tree.train, control = tree.control(nobs = nrow(train), mindev = 0.001))
...
tree.yhat <- predict(prunedTree, newdata = tree.test, type = "class")

# Plot the number of positive duration both in general and in the training and test sets
par(mfrow=c(1,2))
barplot(table(train$y_binary), main = "TRAIN: Number of call with positive and zero dur.", xlab = "Positive duration", ylab = "Frequency")
barplot(table(tree.yhat ), main = "PREDICTED: Number of call with positive and zero dur.", xlab = "Positive duration", ylab = "Frequency")

# Calculate the accuracy of the model with confusion matrix
confusion_matrix <- confusionMatrix(tree.yhat, tree.test$y_binary)
confusion_matrix$overall["Accuracy"]
...
noZeroDf <- train[train$y > 0, ]

# Split the data into training and testing sets
set.seed(1234)

train_index <- createDataPartition(noZeroDf$y, p = 0.7, list = FALSE)
no_zero_train <- noZeroDf[train_index, ]
no_zero_test <- noZeroDf[-train_index, ]


# Train a MARS model with bagEarthGCV using Caret
# Grid search for the best degree
mars_model <- caret::train(y ~ ., data = no_zero_train, method = "gcvEarth", trControl = trainControl(method = "cv", number = 10), tuneGrid = data.frame(degree = 1:5))
summary(mars_model)
# Visualize the MARS model
plot(mars_model)
...
combinedModel <- function(row) {
  y = predict(prunedTree, newdata = sample, type = "class")
  # If the prediction outcome is no traffic, return 0
  if(y == 0)
    as.numeric(0)
  # else return predicted value using regression
  else
    predict(mars_model, newdata = sample)
    
}

sample = train[9, , drop = FALSE]
sample$y = NULL
combinedModel(sample)


# Predict all values of test set
yhat = apply(test, 1, combinedModel)
write.table(file="mySubmission.txt", pmax(0, yhat), row.names = FALSE,col.names = FALSE)
