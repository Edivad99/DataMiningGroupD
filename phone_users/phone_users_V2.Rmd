...
As we can see, the correlation between different months is very high, because they are telling the same story. This means that we can use only the last month data to predict the duration of calls in the tenth month. This will reduce the number of features and make the model more interpretable.
